...
def grade_test_case_llm_judge(test_case, candidate_response: str, provider: str, target_name: str) -> EvaluationResult:
    ...
    class LLMJudge(dspy.Signature):
        ...
        Provide a final score and a brief reasoning.
        """
        expected_outcome = dspy.InputField(desc="The high-level success criterion that the generated_answer should achieve.")
        candidate_response = dspy.InputField(desc="The answer generated by the model being evaluated.")
        
        score = dspy.OutputField(desc="Score from 0.0 to 1.0 indicating how well the response meets the outcome")
        reasoning = dspy.OutputField(desc="A brief explanation for the score, explaining how the generated_answer met or missed the expected_outcome, using the reference_answer for comparison.")
